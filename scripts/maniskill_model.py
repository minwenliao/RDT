...
RoboticDiffusionTransformerModel 类负责初始化和设置模型的各个部分，包括文本编码器、视觉编码器、策略等。
在初始化时，还会加载预训练的文本和视觉模型，之后可以加载模型权重。
获取文本和视觉编码器：

get_text_encoder 和 get_vision_encoder 分别加载文本和视觉的预训练编码器。文本编码器使用的是 T5 模型，而视觉编码器使用的是 SiglipVisionTower。
机器人状态的格式化和动作的解码：

机器人状态（如关节角度）会经过一定的预处理，例如标准化和填充。
通过 _format_joint_to_state 将机器人的关节状态格式化为统一的状态向量，而 _unformat_action_to_joint 则用于将预测的动作转换回机器人的关节角度。
step 方法：

在执行一个动作时，step 方法结合当前的视觉输入（图像）、语言指令（文本嵌入）和机器人状态（如关节角度）来预测机器人应执行的动作。
step 方法的详细解释：
输入：

proprio: 机器人当前的状态（例如关节角度）。
images: 一组图像（通常是来自机器人摄像头的图像）。
text_embeds: 由语言模型编码的文本指令的嵌入。
处理：

图像经过预处理（如调整大小、亮度调整等），并使用视觉编码器生成图像的嵌入。
机器人状态被标准化并格式化成模型可以接受的形式。
语言指令被编码成嵌入，作为模型的输入。
输出：

通过模型预测的动作（如关节角度的变化），并将其解码回机器人实际的动作空间。'''