...
'''新代码中增加了 AutoProcessor 和 AutoModelForVision2Seq 来加载预训练的视觉-语言模型（VLA）。这个模型用来处理输入的视觉信息并预测机器人应该执行的动作。
DATA_STAT 提供了对数据的标准化处理，确保输入的图像数据在输入模型前进行正确的归一化。
训练过程：

新代码通过 tqdm.trange() 来迭代生成多个任务，使用 deque 来保存当前的观测窗口，并且每个任务会迭代若干步来尝试达成任务目标。
通过模型生成的动作来控制机器人执行任务，并且实时渲染视频帧'''